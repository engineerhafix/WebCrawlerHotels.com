// working code populates array of hotel names and urls in the same array
...
//*********************************************************************** */
    ...
    const titles = await page.evaluate(() => {
        const hotelNames = { NameOfTheHotels:
            Array.from(document.querySelectorAll('.uitk-spacing-margin-blockstart-three  div > div > h2'))
            .map((name)=> name.textContent)
        }
          const URL =  {  UrlsOfTheHotels: Array.from(document.querySelectorAll('.uitk-spacing-margin-blockstart-three > div > a'))
            .map((name)=>name.href)
    }
    const user = Object.assign(hotelNames, URL)
            return user;
